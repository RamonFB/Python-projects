...
def dir_treball():
	dir_treball=(tkFileDialog.askdirectory())#Guardem a dir_treball la ruta dels fixers que volem mostrar
...
def ReOmplir():
	...
	os.system('find ' + directoriList.get(0) + ' > x.txt')#find troba els fitxers a la ruta que li dic amb les seues rutes i u guardo al fitxer x.txt
	os.system('sed -i -e "1d" x.txt')#Borrem primera linia
	
	f=open('x.txt' , 'r')#llegeixo el fitxer x.txt
	contingut = f.read()#guardo el continut
	f.close()
...
def TotsDreta():
	fileList.delete(0,END)#Esborro les dues llistes
	fileList2.delete(0,END)
	os.system('find ' + directoriList.get(0) + ' > x.txt')#find troba els fitxers a la ruta que li dic amb les seues rutes i u guardo al fitxer x.txt
	os.system('sed -i -e "1d" x.txt')#Borrem primera linia
	f=open('x.txt' , 'r')
	for text in f.readlines()[:]:#Ho faig d'aquesta manera perque readlines[:] fa un salt de linia a cada fixer
...
#Funcio per fer la compresio
def crear():
	if checked_cap.get() == 0 and checked_bzip2.get() == 0 and checked_gzip.get() == 0 and checked_xz.get() == 0:#Comprovo si no hi ha cap metode de compressio escollit
		tkMessageBox.showinfo("Error","Escull algun metode de compressio")#Sino hi ha cap salta el missatge
		
	fitxers = ""#string per guardar els fitxers
	f=open("out.txt",'r')#obrec el fitxer
	for text in f.readlines()[:]:
		text2=text	#Ruta inicial dels fitxers
	f.close()
	
	if checked_cap.get():#Si el boto checked_cap esta seleccionat
		
		Crear=tkFileDialog.asksaveasfilename()	#Con asksaveasfilename on comptes de asksaveasfile et dona nomes un string amb la ruta+nom del fitxer
		Crear2 = Crear.split('/')[-1]#Tallo per la ultima barra per tant Crear2 sera el nom que li donare al .tar
		
		if Crear is None: #Si premem cancelar
			return
		...
		comanda = ("tar -cf " + Crear2 + ".tar -C " + text2 + fitxers)#String per mostrar al listbox del boto executar
		
		def comprimir():#funcio que cridem quan li donem al boto executar
			os.system('tar -cf ' + Crear2 + '.tar -C ' + text2 + fitxers)#comanda que utilitzarem per comprimir els fitxers -C fa que pugui comprimir els fitxers desde un altre directori
		...
		B1FC = Button(FinestraExecutar,text= "Executar", command=comprimir).pack(side=RIGHT)
		direccionList = Listbox(FinestraExecutar,yscrollcommand=sc1f3.set,width=60,bg='#FFFFFF',height=1)
		direccionList.pack(side=TOP,anchor=W,expand=TRUE,fill=BOTH)
		direccionList.insert(END,comanda)
	
	if checked_bzip2.get():#Si el boto checked_cap esta seleccionat
		...
		comanda = ("tar -cjf " + Crear2 + ".tbz -C " + text2 + fitxers)#String per mostrar al listbox del boto executar
		
		def comprimir():#funcio que cridem quan li donem al boto executar
			os.system('tar -cjf ' + Crear2 + '.tbz -C ' + text2 + fitxers)#comanda que utilitzarem per comprimir els fitxers -C fa que pugui comprimir els fitxers desde un altre directori
	...
	if checked_gzip.get():#Si el boto checked_cap esta seleccionat
		...
		comanda = ("tar -czf " + Crear2 + ".tgz -C " + text2 + fitxers)#String per mostrar al listbox del boto executar
		
		def comprimir():#funcio que cridem quan li donem al boto executar
			os.system('tar -czf ' + Crear2 + '.tgz -C ' + text2 + fitxers)#comanda que utilitzarem per comprimir els fitxers -C fa que pugui comprimir els fitxers desde un altre directori
	...
	if checked_xz.get():#Si el boto checked_cap esta seleccionat
		...
		comanda = ("tar -cJf " + Crear2 + ".txz -C " + text2 + fitxers)#String per mostrar al listbox del boto executar
		
		def comprimir():#funcio que cridem quan li donem al boto executar
			os.system('tar -cJf ' + Crear2 + '.txz -C ' + text2 + fitxers)#comanda que utilitzarem per comprimir els fitxers -C fa que pugui comprimir els fitxers desde un altre directori
...
def sortir():
	os.system('rm a.txt x.txt out.txt output.txt')#esborro els fitxers creats
	window.quit()#tanco la finestra

#BOTO DIRECTORI + LISTBOX ON ES VISUALITZA EL DIRECTORI
f1=Frame(window)
l1f1 = Button(f1,text="Escollir directori treball", command=dir_treball).pack(side = LEFT)
...
l2f1=Label(f1,text="	  	      Fitxers a incorporar al fitxer tar:").pack(side=LEFT)

f1.pack(side=TOP,anchor=W)
######################################################

#Finestra 2 amb els botons i els Checkbuttons
f2=Frame(window)
l1f2=Label(f2,text="Llista:")
b1f2=Button(f2,text="Reomplir",command=ReOmplir)
b2f2=Button(f2,text="Ocultar NO seleccionats", command=ocultaNoSelec)
b3f2=Button(f2,text="Ocultar seleccionat",command=ocultaSelec)

l3f2=Label(f2,text="		          Compressio:")
ch1f2=Checkbutton (f2,text="Cap",onvalue=1,offvalue=0, variable=checked_cap)
ch2f2=Checkbutton (f2,text="gzip",onvalue=1,offvalue=0, variable=checked_gzip)
ch3f2=Checkbutton (f2,text="bzip2",onvalue=1,offvalue=0, variable=checked_bzip2)
ch4f2=Checkbutton (f2,text="xz",onvalue=1,offvalue=0, variable=checked_xz)
...
b3f2.pack(side=LEFT)
l3f2.pack(side=LEFT)
ch1f2.pack(side=LEFT)
ch2f2.pack(side=LEFT)
ch3f2.pack(side=LEFT)
ch4f2.pack(side=LEFT)
...
b1f5=Button(f12,text="Crea",command=crear).pack(side=LEFT)
l1f5=Label(f12,text="Fitxer tar:").pack(side=LEFT)
